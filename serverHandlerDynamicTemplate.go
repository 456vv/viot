...
var errTemplateNotParse =  errors.New("viot: The template has not been parsed yet!")
...
func (h *shdtHeader) openIncludeFile(rootPath, pagePath string) (map[string]string, error){
	...
	for _, v := range h.filePath {
		...
		if err != nil {
			return nil, fmt.Errorf("viot: Dynamically embedded template file read failed(%s)", err.Error())
		}
...
func (T *serverHandlerDynamicTemplate) separation(buf *bufio.Reader) (shdtHeader, string, error) {
    ...
    for {
        ...
        if i < 0 {
        	return shdtHeader{}, "", fmt.Errorf("viot: Error parsing file header(%s)", string(line))
    	}
    ...
    if err != nil {
    	return shdtHeader{}, "", fmt.Errorf("viot: Error reading file body data(%s)", err.Error())
    }
...
func (T *serverHandlerDynamicTemplate) loadTmpl(delimLeft, delimRight string, t *template.Template, f map[string]string) (*template.Template, error) {
    ...
    if t == nil {
        return t, errors.New("viot: The parent template is nil")
    }
    for k, v := range f {
        tmpl = t.New(k)
